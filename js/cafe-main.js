...
// =========================================================
// 2. DOM 로드 후 초기화 및 이벤트 바인딩
// =========================================================
document.addEventListener("DOMContentLoaded", () => {
  // DOM 요소 캐싱
  elements = {
    themeToggle: document.getElementById("theme-toggle"),
    sidebar: document.getElementById("cafe-sidebar"),
    btnToggle: document.getElementById("btn-sidebar-toggle"),
    btnClose: document.getElementById("btn-sidebar-close"),
    ...
    sidebarSearchBtn: document.querySelector(".sidebar-search button")
  };
  ...
  if (elements.themeToggle) {
    elements.themeToggle.addEventListener("change", () => {
      const nextTheme = elements.themeToggle.checked ? "light" : "dark";
      applyTheme(nextTheme);
      localStorage.setItem("user-theme", nextTheme);
    });
  }
  ...
  // 사이드바 토글 이벤트
  if (elements.btnToggle) elements.btnToggle.addEventListener("click", openSidebar);
  if (elements.btnClose) elements.btnClose.addEventListener("click", closeSidebar);
  ...
  if (elements.sidebarSearchBtn) {
    elements.sidebarSearchBtn.addEventListener("click", searchFromSidebar);
  }
...
// =========================================================
function renderBoard(categoryFilter = "all", searchQuery = "") {
  if (!elements.boardBody) return;

  const posts = Object.values(boardData).sort((a, b) => {
    const aIsNotice = a.category === "notice";
    const bIsNotice = b.category === "notice";

    if (aIsNotice !== bIsNotice) return aIsNotice ? -1 : 1;
    return b.id - a.id;
  });
  ...
  const html = posts.map(post => {
    ...
    return `
      <tr ${rowClass} data-id="${post.id}">
        <td class="col-cat">${catBadge}</td>
        <td class="col-title">
          <a href="#none" onclick="openPost(event, ${post.id})">
            ${escapeHtml(post.title)}
          </a>
        </td>
        <td class="col-writer">${escapeHtml(post.writer)}</td>
  ...
  elements.boardBody.innerHTML = html || `<tr><td colspan="5" style="text-align:center; padding: 40px 0; color: var(--text-sub);">검색 결과가 없습니다.</td></tr>`;
  if (elements.totalCountEl) elements.totalCountEl.textContent = renderedCount;
}
...
function filterBoard(category, event) {
  ...
  document.querySelectorAll(".btn-filter-tab").forEach(btn => {
    ...
    const isActive = btnOnClick.includes(`'${category}'`);
    btn.classList.toggle("active", isActive);
  });

  // 2) 사이드바 메뉴 활성화 동기화
  document.querySelectorAll(".menu-tree a").forEach(link => {
    const linkOnClick = link.getAttribute("onclick") || "";
    const isActive = linkOnClick.includes(`'${category}'`);
...
// =========================================================
function searchFromSidebar() {
  if (!elements.sidebarSearchInput) return;

  const query = elements.sidebarSearchInput.value;
  renderBoard(currentCategory, query);
  closeSidebar();
}
